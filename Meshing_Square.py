...
# ===============================================================
# (A) Local-neighborhood lambda propagation
# ===============================================================
...
# ===============================================================
# (C) Assign cell size from lambda
# ===============================================================
...
# ===============================================================
# (D) Adaptive refinement (Delaunay)
# ===============================================================
...
# ===============================================================
# (E2) ADAPTIVE SQUARE BLUFF BODY (ROTATED)
# ===============================================================
def create_square_body_adaptive(center, side, angle_deg, get_length):
    ...
    verts = base @ R.T + np.array([cx, cy])

    # Adaptive sampling along edges
    sampled = []
    for i in range(4):
        ...
        while t < 1.0:
            ...
            probe = pt + 0.01 * normal

            hloc = get_length(probe)
            dt = hloc / L
            sampled.append(pt)
            t += dt

    sampled = np.array(sampled)
    return sampled, Polygon(verts[:-1])
...
# ===============================================================
# (F) Filtering & boundary classification
# ===============================================================
...
def write_fluent(filename, points2d, triangles, boundaries, height):
    ...
    with open(filename, "w") as f:
        ...
        for phys_id, (name, edges) in enumerate(boundaries.items(), 2):
            for e in edges:
                a, b = e
                f.write(f"{eid} 3 2 {phys_id} {phys_id} "
                        f"{a+1} {b+1} {b+1+n} {a+1+n}\n")
                eid += 1
...
def generate_mesh(config):
    ...
    bluff_pts, exclusion = create_square_body_adaptive(
        config["bluff_body_center"],
        config["bluff_body_size"],
        config["bluff_body_angle"],
        get_length
    )
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "output_file": "square_bluff_mesh.msh",

        "bluff_body_center": (4.5, 4.5),
        "bluff_body_size": 1.0,
        "bluff_body_angle": 22.0,

        "initial_spacing": 0.2,
        "max_iter": 15,

        "extrusion_height": 1.0,
        "domain_bounds": (0, 13, 0, 9),
